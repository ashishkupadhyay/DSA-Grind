# DSA Prep Tracker

Legend: `☐` = not marked yet, `✓` = solved, `✗` = not solved.



| Category | LeetCode # | Question | Remark |
|---|---:|---|---|
| Arrays | 1 | Two Sum | ✓ |
| Arrays | 121 | Best Time to Buy and Sell Stock |✓ |
| Arrays | 217 | Contains Duplicate | ✓ |
| Arrays | 238 | Product of Array Except Self | ✓ |
| Arrays | 53 | Maximum Subarray | ☐ |
| Arrays | 152 | Maximum Product Subarray | ☐ |
| Arrays | 283 | Move Zeroes |✓ |
| Arrays | 26 | Remove Duplicates from Sorted Array | ✓ |
| Arrays | 189 | Rotate Array | ✓|
| Arrays | 153 | Find Minimum in Rotated Sorted Array | ☐ |
| Arrays | 33 | Search in Rotated Sorted Array | ☐ |
| Arrays | 169 | Majority Element | ✓ |
| Arrays | 268 | Missing Number | ✓ |
| Arrays | 448 | Find All Numbers Disappeared in an Array |  ✓|
| Sliding Window | N/A | Maximum Sum Subarray of Size K | ☐ |
| Sliding Window | 3 | Longest Substring Without Repeating Characters | ✓ |
| Sliding Window | 424 | Longest Repeating Character Replacement | ☐ |
| Sliding Window | 209 | Minimum Size Subarray Sum | ☐ |
| Sliding Window | 567 | Permutation in String | ☐ |
| Sliding Window | 438 | Find All Anagrams in a String | ✓ |
| Sliding Window | 239 | Sliding Window Maximum | ☐ |
| Binary Search | N/A | Binary Search | ☐ |
| Binary Search | 34 | Find First and Last Position of Element in Sorted Array | ☐ |
| Binary Search | 35 | Search Insert Position | ☐ |
| Binary Search | 162 | Find Peak Element | ☐ |
| Binary Search | 153 | Find Minimum in Rotated Sorted Array | ☐ |
| Binary Search | 33 | Search in Rotated Sorted Array | ☐ |
| Binary Search | 4 | Median of Two Sorted Arrays | ☐ |
| Hashing | 1 | Two Sum | ☐ |
| Hashing | 560 | Subarray Sum Equals K | ☐ |
| Hashing | 128 | Longest Consecutive Sequence | ☐ |
| Hashing | 347 | Top K Frequent Elements | ☐ |
| Hashing | 242 | Valid Anagram | ☐ |
| Hashing | 49 | Group Anagrams | ☐ |
| Hashing | 205 | Isomorphic Strings | ☐ |
| Stack | 20 | Valid Parentheses | ☐ |
| Stack | 155 | Min Stack | ☐ |
| Stack | 496 | Next Greater Element I | ☐ |
| Stack | 739 | Daily Temperatures | ☐ |
| Stack | 84 | Largest Rectangle in Histogram | ☐ |
| Stack | 150 | Evaluate Reverse Polish Notation | ☐ |
| Queue | 232 | Implement Queue using Stacks | ☐ |
| Queue | 225 | Implement Stack using Queues | ☐ |
| Queue | 239 | Sliding Window Maximum | ☐ |
| Queue | N/A | First Non-Repeating Character in Stream | ☐ |
| Linked List | 206 | Reverse Linked List | ☐ |
| Linked List | 141 | Linked List Cycle | ☐ |
| Linked List | 876 | Middle of the Linked List | ☐ |
| Linked List | 21 | Merge Two Sorted Lists | ☐ |
| Linked List | 19 | Remove Nth Node From End of List | ☐ |
| Linked List | 160 | Intersection of Two Linked Lists | ☐ |
| Linked List | 92 | Reverse Linked List II | ☐ |
| Linked List | 138 | Copy List with Random Pointer | ☐ |
| Linked List | 146 | LRU Cache | ☐ |
| Trees | 104 | Maximum Depth of Binary Tree | ☐ |
| Trees | 100 | Same Tree | ☐ |
| Trees | 226 | Invert Binary Tree | ☐ |
| Trees | 102 | Binary Tree Level Order Traversal | ☐ |
| Trees | 543 | Diameter of Binary Tree | ☐ |
| Trees | 110 | Balanced Binary Tree | ☐ |
| Trees | 572 | Subtree of Another Tree | ☐ |
| Trees | 235 | Lowest Common Ancestor of a BST | ☐ |
| Trees | 236 | Lowest Common Ancestor of a Binary Tree | ☐ |
| Trees | 98 | Validate Binary Search Tree | ☐ |
| Trees | 230 | Kth Smallest Element in a BST | ☐ |
| Trees | 105 | Construct Binary Tree from Preorder and Inorder Traversal | ☐ |
| Greedy | 55 | Jump Game | ☐ |
| Greedy | 45 | Jump Game II | ☐ |
| Greedy | 134 | Gas Station | ☐ |
| Greedy | N/A | Activity Selection Problem | ☐ |
| Greedy | 56 | Merge Intervals | ☐ |
| Greedy | 435 | Non-overlapping Intervals | ☐ |
| Intervals | 57 | Insert Interval | ☐ |
| Intervals | 56 | Merge Intervals | ☐ |
| Intervals | 252 | Meeting Rooms | ☐ |
| Intervals | 253 | Meeting Rooms II | ☐ |
| Backtracking | 78 | Subsets | ☐ |
| Backtracking | 90 | Subsets II | ☐ |
| Backtracking | 46 | Permutations | ☐ |
| Backtracking | 47 | Permutations II | ☐ |
| Backtracking | 39 | Combination Sum | ☐ |
| Backtracking | 40 | Combination Sum II | ☐ |
| Backtracking | 22 | Generate Parentheses | ☐ |
| Backtracking | 79 | Word Search | ☐ |
| Graphs | 200 | Number of Islands | ☐ |
| Graphs | 133 | Clone Graph | ☐ |
| Graphs | 733 | Flood Fill | ☐ |
| Graphs | 207 | Course Schedule | ☐ |
| Graphs | 210 | Course Schedule II | ☐ |
| Graphs | N/A | Detect Cycle in Graph | ☐ |
| Graphs | 323 | Number of Connected Components in an Undirected Graph | ☐ |
| Graphs | 994 | Rotting Oranges | ☐ |
| Dynamic Programming | 70 | Climbing Stairs | ☐ |
| Dynamic Programming | 198 | House Robber | ☐ |
| Dynamic Programming | 213 | House Robber II | ☐ |
| Dynamic Programming | 322 | Coin Change | ☐ |
| Dynamic Programming | 300 | Longest Increasing Subsequence | ☐ |
| Dynamic Programming | 1143 | Longest Common Subsequence | ☐ |
| Dynamic Programming | N/A | 0/1 Knapsack | ☐ |
| Dynamic Programming | 416 | Partition Equal Subset Sum | ☐ |
| Dynamic Programming | 72 | Edit Distance | ☐ |
